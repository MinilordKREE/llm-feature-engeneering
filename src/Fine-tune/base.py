    ...
    def __init__(self, use_case: Enum, model_name: str, batch_size: int = 100, **kwargs):
        ...
        try:
            if self.model_name == "fine_tuned_model":
                self.__model = AutoModel.from_pretrained("/data/chenxi/llm-feature-engeneering/src/Fine-tune/circor/model/checkpoint-1140", **kwargs).to(self.device)
            elif self.model_name == "fine_tuned_model_0":
                self.__model = AutoModel.from_pretrained("/home/chenxi/data/newplan/finetune/cross/0/model_name/checkpoint-1140", **kwargs).to(self.device)
            elif self.model_name == "fine_tuned_model_1":
                self.__model = AutoModel.from_pretrained("/home/chenxi/data/newplan/finetune/cross/1/model_name/checkpoint-1140", **kwargs).to(self.device)
            elif self.model_name == "fine_tuned_model_2":
                self.__model = AutoModel.from_pretrained("/home/chenxi/data/newplan/finetune/cross/2/model_name/checkpoint-1140", **kwargs).to(self.device)
            elif self.model_name == "fine_tuned_model_3":
                self.__model = AutoModel.from_pretrained("/home/chenxi/data/newplan/finetune/cross/3/model_name/checkpoint-1140", **kwargs).to(self.device)
            elif self.model_name == "fine_tuned_model_4":
                self.__model = AutoModel.from_pretrained("/home/chenxi/data/newplan/finetune/cross/4/model_name/checkpoint-1140", **kwargs).to(self.device)
            else:
                self.__model = AutoModel.from_pretrained(self.model_name, **kwargs).to(self.device)
        except OSError:
            raise ValueError(f"Unable to load model from {model_name}")
